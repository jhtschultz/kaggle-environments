async function renderer(context) {
  const {
    environment,
    // frame, // 'frame' is available in context but not used in this renderer
    height = 400,
    parent,
    step,
    width = 400,
  ...
  // --- Piece Image Data (Base64 encoded) ---
  // User will fill this in.
  // IMPORTANT: Keys should be 'p', 'r', 'n', 'b', 'q', 'k' for white pieces
  // and 'P', 'R', 'N', 'B', 'Q', 'K' for black pieces.
  ...
  // --- Initialize Piece Image Objects (Ensures it runs only once) ---
  const pieceImages = {};
  let imagesLoadedCount = 0;
  const totalImagesToLoad = Object.keys(pieceImagesSrc).length;
  let allImagesConsideredLoaded = totalImagesToLoad === 0;
  
  // Check if images were already initialized (e.g., by a previous call to renderer for a different step)
  // A more robust way for truly global init would be outside the renderer or via a global flag.
  // For this self-contained renderer, we use a property on the parent to track init.
  const imageInitFlag = `__pieceImagesInitialized_${parent.id || 'global'}`;

  function imageLoadCallback(pieceCode, success) {
    imagesLoadedCount++;
    if (!success) {
      console.warn(`Failed to load image for piece: ${pieceCode}. Will use symbol fallback.`);
    }
    if (imagesLoadedCount === totalImagesToLoad) {
      allImagesConsideredLoaded = true;
      console.log("All piece images processed (loaded or errored). Attempting redraw if necessary.");
      // Trigger a redraw of the current step, as images are now ready (or failed)
      const currentCanvas = parent.querySelector("canvas.chess-canvas-renderer");
      if (currentCanvas && environment.steps && environment.steps[step]) {
          renderBoardAndPieces(currentCanvas.getContext("2d"), getFENFromStepData(environment.steps[step], step, "image_ready_redraw"));
      }
    }
  }

  if (!parent[imageInitFlag] && totalImagesToLoad > 0) {
    console.log("Initializing piece images for the first time...");
    for (const pieceCode in pieceImagesSrc) {
      if (pieceImagesSrc.hasOwnProperty(pieceCode)) {
        const img = new Image();
        (function(pCode) { // IIFE to capture pieceCode
            img.onload = () => imageLoadCallback(pCode, true);
            img.onerror = () => imageLoadCallback(pCode, false);
        })(pieceCode);
        img.src = pieceImagesSrc[pieceCode];
        pieceImages[pieceCode] = img;
      }
    }
    parent[imageInitFlag] = true; // Mark as initialized for this parent
  } else if (totalImagesToLoad === 0) {
      allImagesConsideredLoaded = true; // No images to load
  }
  ...
  function drawPiece(ctx, type, color, x, y, size) {
    ...
    if (img && img.complete && img.naturalWidth > 0) {
      ...
      const symbol = pieceSymbols[pieceCode] || '?';
      ctx.font = `${size * 0.75}px Arial`; // Slightly smaller for symbols
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.fillStyle = (color === 'w') ? '#f0f0f0' : '#333333'; // High contrast
      ctx.fillText(symbol, x + size / 2, y + size / 2 + size * 0.05); // Y-offset for better symbol centering
    }
  }

  // --- Common Dimensions & Canvas Setup ---
  const canvasSize = Math.min(height, width);
  const boardSize = canvasSize * 0.8;
  const squareSize = boardSize / 8;
  const offset = (canvasSize - boardSize) / 2;

  let canvas = parent.querySelector("canvas.chess-canvas-renderer");
  if (!canvas) {
    canvas = document.createElement("canvas");
    canvas.classList.add("chess-canvas-renderer");
  ...
  if (typeof Chess === "undefined") {
    parent.innerHTML = "<p style='color:red; font-family: sans-serif;'>Error: chess.js library not found.</p>";
    return;
  }

  // --- Function to get the FEN string from step data ---
  function getFENFromStepData(stepDataArray, forStep, callSource = "unknown") {
    console.log(`getFENFromStepData [${callSource}]: Called for step ${forStep}.`);
    if (!stepDataArray || stepDataArray.length === 0) {
        console.warn(`getFENFromStepData [${callSource}]: No stepDataArray for step ${forStep}.`);
        return null;
    }
    
    let fenToReturn = null;

    // Try Game Master (last agent) observation_string
    const gameMasterAgentIndex = stepDataArray.length - 1;
    if (gameMasterAgentIndex >= 0 && stepDataArray[gameMasterAgentIndex] && stepDataArray[gameMasterAgentIndex].observation) {
        const gmObs = stepDataArray[gameMasterAgentIndex].observation;
        if (gmObs.observation_string && typeof gmObs.observation_string === 'string') {
            const obsStr = gmObs.observation_string.trim();
            if (obsStr.includes('/')) { // Quick check for FEN-like string
                if (obsStr.startsWith("{") && obsStr.endsWith("}")) {
                    try {
                        const parsedJson = JSON.parse(obsStr);
                        if (parsedJson.fen && typeof parsedJson.fen === 'string' && parsedJson.fen.includes('/')) {
                            fenToReturn = parsedJson.fen;
                            console.log(`getFENFromStepData [${callSource}]: FEN from GM JSON 'fen': ${fenToReturn}`);
                        } else if (parsedJson.board && typeof parsedJson.board === 'string' && parsedJson.board.includes('/')) {
                            fenToReturn = parsedJson.board;
                            console.log(`getFENFromStepData [${callSource}]: FEN from GM JSON 'board': ${fenToReturn}`);
                        }
                    } catch (e) { if (obsStr.includes('/')) fenToReturn = obsStr; } // Fallback to treating as direct FEN if JSON parse fails
                } else { fenToReturn = obsStr; } // Assume direct FEN
                if (fenToReturn) console.log(`getFENFromStepData [${callSource}]: Found FEN in GM obs_string: ${fenToReturn}`);
            }
        }
    }

    // Fallback: Player 0's observation.board (Kaggle chess.js specific)
    if (!fenToReturn && stepDataArray[0]?.observation?.board) {
        const p0Board = stepDataArray[0].observation.board;
        if (typeof p0Board === 'string' && p0Board.includes('/')) {
            fenToReturn = p0Board;
            console.log(`getFENFromStepData [${callSource}]: FEN from Player 0 'board': ${fenToReturn}`);
        }
    }
    
    // Fallback: Player 0's observation.observation_string
    if (!fenToReturn && stepDataArray[0]?.observation?.observation_string) {
        const p0ObsString = stepDataArray[0].observation.observation_string.trim();
        if (p0ObsString.includes('/')) { // Similar FEN check as above
            if (p0ObsString.startsWith("{") && p0ObsString.endsWith("}")) {
                 try {
                    const parsedJson = JSON.parse(p0ObsString);
                    if (parsedJson.fen && typeof parsedJson.fen === 'string' && parsedJson.fen.includes('/')) fenToReturn = parsedJson.fen;
                    else if (parsedJson.board && typeof parsedJson.board === 'string' && parsedJson.board.includes('/')) fenToReturn = parsedJson.board;
                } catch(e) { if (p0ObsString.includes('/')) fenToReturn = p0ObsString; }
            } else { fenToReturn = p0ObsString; }
            if (fenToReturn) console.log(`getFENFromStepData [${callSource}]: Found FEN in Player 0 obs_string: ${fenToReturn}`);
        }
    }
    
    if (!fenToReturn) console.error(`getFENFromStepData [${callSource}]: FEN NOT FOUND for step ${forStep}.`);
    return fenToReturn;
  }

  // --- PGN Button Logic ---
  const pgnButtonId = `copy-pgn-button-${parent.id || 'canvas'}-${Date.now()}`;
  let downloadButton = parent.querySelector(`#${pgnButtonId}`);

  if (!downloadButton && environment.steps && environment.steps.length > 0) {
    let initialFEN = null;
    let pgnGame = null;

    if (environment.steps[0]) {
        initialFEN = getFENFromStepData(environment.steps[0], 0, "PGN_INIT");
    }

    if (initialFEN) {
        try { pgnGame = new Chess(initialFEN); }
        catch (e) { console.error("PGN: Failed to initialize chess.js with initialFEN:", initialFEN, e); initialFEN = null; /* Force default below */ }
    }
    
    if (!initialFEN && environment.steps[0] && environment.steps[0][0] && environment.steps[0][0].observation && environment.steps[0][0].observation.step === 0) {
        // If step 0 FEN is explicitly null/invalid, default to standard starting position for PGN.
        console.warn("PGN Exporter: Initial FEN from step 0 was null or invalid. Defaulting PGN to standard starting position.");
        try { pgnGame = new Chess(); initialFEN = pgnGame.fen(); } // Use default and get its FEN
        catch (e) { console.error("PGN: Failed to initialize chess.js with default FEN:", e); pgnGame = null; }
    }

    if (pgnGame) {
      try {
        const info = environment.info || {};
        const agent1Name = info?.TeamNames?.[0] || "Agent 0";
        const agent2Name = info?.TeamNames?.[1] || "Agent 1";
        pgnGame.header( /* ... PGN Headers as before ... */ );
        
        for (let i = 1; i <= step && i < environment.steps.length; i++) {
          // ... (PGN move application logic as before) ...
        }
        // ... (PGN result header logic as before) ...
        let pgnString = pgnGame.pgn();
        // ... (PGN button creation and event listener as before, using pgnString) ...
        // Ensure this part is robust and doesn't cause a blank screen if it fails
        downloadButton = document.createElement("button");
        downloadButton.id = pgnButtonId;
        downloadButton.textContent = "Copy PGN";
        downloadButton.style.cssText = "position: absolute; top: 5px; left: 5px; z-index: 100; padding: 5px 10px; font-size: 12px; cursor: pointer; background-color: #f0f0f0; border: 1px solid #ccc; border-radius: 3px;";
        parent.appendChild(downloadButton);
        downloadButton.addEventListener("click", async () => { /* ... click handler as before ... */ });

      } catch (e) {
        console.error("Error during PGN button contents setup (after FEN confirmed):", e);
        if (downloadButton && downloadButton.parentElement) { // Clean up if button was partially made
            downloadButton.parentElement.removeChild(downloadButton);
        }
      }
    } else {
        console.error("PGN Exporter: PGN generation skipped as game could not be initialized (initialFEN likely invalid/missing and default failed).");
    }
  }
  ...
  // Draw Board Squares
  for (let r = 0; r < 8; r++) {
    for (let c_col = 0; c_col < 8; c_col++) {
      const x = c_col * squareSize + offset;
      const y = r * squareSize + offset;
      ctx.fillStyle = (r + c_col) % 2 === 0 ? "#FFCE9E" : "#D18B47";
      ctx.fillRect(x, y, squareSize, squareSize);
    }
  }
  
  // --- Function to render board and pieces (called below) ---
  function renderBoardAndPieces(canvasCtx, fen) {
      if (!fen) {
          let message = `Board state (FEN) unavailable for step ${step}.`;
          if (step === 0 && (!environment.steps || !environment.steps[0])) {
              message = "Waiting for game to start...";
          }
          console.warn(message);
          canvasCtx.font = `bold ${Math.max(12, Math.round(0.04 * canvasSize))}px sans-serif`;
          canvasCtx.fillStyle = step === 0 ? "#555" : "#A00";
          canvasCtx.textAlign = "center";
          canvasCtx.fillText(message, canvasSize / 2, canvasSize / 2);
          return false; // Indicate failure to render board
      }
      try {
        ...
        for (let r_idx = 0; r_idx < 8; r_idx++) {
          for (let c_idx = 0; c_idx < 8; c_idx++) {
            const pieceObj = boardArray[r_idx][c_idx];
            if (pieceObj) {
              const x = c_idx * squareSize + offset;
              const y = r_idx * squareSize + offset;   
              drawPiece(canvasCtx, pieceObj.type, pieceObj.color, x, y, squareSize);
            }
          }
        }
        return true; // Indicate success
      } catch (e) {
          console.error(`Error initializing chess.js or drawing pieces with FEN: ${fen}`, e);
          canvasCtx.font = `bold ${Math.max(12, Math.round(0.04 * canvasSize))}px sans-serif`;
          canvasCtx.fillStyle = "#A00";
          canvasCtx.textAlign = "center";
          canvasCtx.fillText("Error rendering board state.", canvasSize / 2, canvasSize / 2);
          return false; // Indicate failure
      }
  }
  
  const boardFENForDisplay = (environment.steps && environment.steps[step]) ? getFENFromStepData(environment.steps[step], step, "DISPLAY") : null;

  // Draw Status Text
  // (Status text logic as before, using boardFENForDisplay to initialize a local Chess instance for turn/game_over checks)
  // ... (Ensure this part is also robust to boardFENForDisplay being null)

  // Draw Pieces
  const boardRenderedSuccessfully = renderBoardAndPieces(ctx, boardFENForDisplay);

  // If images were not loaded initially and board was rendered (even with symbols), 
  // and now images ARE loaded, trigger one more pass.
  // This is tricky because renderer gets called per step.
  // The imageLoadCallback now handles its own redraw attempt if necessary.
}