...
for name in listdir(utils.envs_path):
    if name not in IMPORT_ENVS:
        print(f"Skipping importing env: {name}")
        continue
    if name == "open_spiel":
        os_module = import_module(f".envs.{name}.{name}", __name__)
        for env in os_module.envs:
            _register_env(name, env)
    try:
